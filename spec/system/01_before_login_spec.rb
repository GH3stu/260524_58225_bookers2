...
describe '[STEP1] ユーザログイン前のテスト' do
  ...
  describe 'ヘッダーのテスト: ログインしていない場合' do
    ...
    context '表示内容の確認' do
      it 'Bookersリンクが表示される: 左上から1番目のリンクが「Bookers」である', spec_category: "CRUD機能に対するコントローラの処理と流れ(ログイン状況を意識した応用)" do
        home_link = find_all('a')[0].text
        expect(home_link).to match(/Bookers/)
      end
      it 'Homeリンクが表示される: 左上から2番目のリンクが「Home」である', spec_category: "CRUD機能に対するコントローラの処理と流れ(ログイン状況を意識した応用)" do
        home_link = find_all('a')[1].text
        expect(home_link).to match(/Home/)
      end
      it 'Aboutリンクが表示される: 左上から3番目のリンクが「About」である', spec_category: "CRUD機能に対するコントローラの処理と流れ(ログイン状況を意識した応用)" do
        about_link = find_all('a')[2].text
        expect(about_link).to match(/About/)
      end
      it 'Sign upリンクが表示される: 左上から4番目のリンクが「Sign up」である', spec_category: "CRUD機能に対するコントローラの処理と流れ(ログイン状況を意識した応用)" do
        signup_link = find_all('a')[3].text
        expect(signup_link).to match(/Sign up/)
      end
      it 'Log inリンクが表示される: 左上から5番目のリンクが「Log in」である', spec_category: "CRUD機能に対するコントローラの処理と流れ(ログイン状況を意識した応用)" do
  ...
  describe 'ユーザログイン' do
    ...
    context '表示内容の確認' do
      ...
      it 'email_addressフォームは表示されない', spec_category: "deviseの基本的な導入・認証設定" do
        expect(page).not_to have_field 'email_address'
      end
    end
